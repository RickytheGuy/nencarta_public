...
class NencartaConfig:
    ...
    def validate_user_floodmaps(self):
        ...
        user_flow_files = self.get("user_flow_files", None)
        if isinstance(user_flow_files, str):
            user_flow_files = [user_flow_files]
            
        if floodmap_mode == FloodMapMode.USER and (not user_flow_files or not isinstance(user_flow_files, list) or len(user_flow_files) < 1):
    ...
    def setup(self):
        ...
        self.floodmap_id = self.get('floodmap_identifier', '')
        if self.floodmap_id:
        ...
        self.dem_dir: str = self.get_path("dem_dir", None)
        ...
        self.new_strm_threshold_km2: float = self.get("new_strm_threshold_km2")
        ...
        self.buffer_distance_degrees: float = self.get("buffer_distance_degrees", 0.05)
        ...
        self.folder_structure: FolderStructure = self.get("FolderStructure", None)
