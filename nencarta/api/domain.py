...
from nencarta.flood_folder import FolderStructure
...
from nencarta._constants import (
    ESA_TILES_FILE, GEOGLOWS_DAILY_URL, GEOGLOWS_FDC_URL, GEOGLOWS_RETURN_PERIODS_URL, NWM_RP_URL, CACHED_ESA_GRID
)
...
class Domain():
    ...
    def dem_basename(self) -> str:
        ...
        return os.path.splitext(os.path.basename(self.dem))[0]
    ...
    def make_stream_raster(self, 
        ...
        stream_ds: gdal.Dataset = gdal.GetDriverByName('GTiff').Create(self.stream_raster, dem_raster.ncols, dem_raster.nrows, 1, gdal.GDT_Int32, options=['COMPRESS=DEFLATE', 'PREDICTOR=2'])
    ...
    def define_configs_for_dem_cleaning(self,
        ...
        params["FS_ADJUST_FLOW_BY_FRACTION"] = 1
    ...
    def define_arc_configs(self, 
        ...
        if not disable_bathymetry:
            ...
            params["Bathy_Use_Banks"] = bathy_use_banks
            params["Make_Output_GPKG"] = bathy_args.get("Make_Output_GPKG", True)
            if mapper.is_curve2flood_fldpln_mapper():
                params.update(self._fldpln_inputs())
            ...
            if use_specified_depths:
                params["FloodSpreader_SpecifyDepth"] = specified_depth
            # else:
            #     if not self.initial_floodmap_for_clean_dem:
            #         raise ValueError("Floodmap for clean DEM must be generated before defining ARC configs without specified depths.")
            #     params["BathyWaterMask"] = self.initial_floodmap_for_clean_dem
    ...
    def define_c2f_configs(self, 
        ...
        if flood_lc_and_streams or velocity_name:
            if not self.land_cover:
    ...
    def make_clean_dem(self, 
        ...
        if not os.path.exists(self.curve_file_for_clean_dem):
